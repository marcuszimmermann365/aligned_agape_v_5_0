...
class GHIAdapter(Adapter):
    name, source = "ghi_index", "Global Hunger Index"
    def fetch(self) -> DataPoint:
        ...
        for k,v in cols.items():
            if "ghi" in k or "score" in k or "index" in k:
                score = v; break
        if year is None or score is None:
            raise ValueError("Missing Year/Score columns for GHI.")
