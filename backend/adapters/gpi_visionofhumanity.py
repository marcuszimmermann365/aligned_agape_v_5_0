...
class GPIGlobalAdapter(Adapter):
    name, source = "gpi_global_avg", "Vision of Humanity / Institute for Economics & Peace"
    def fetch(self) -> DataPoint:
        ...
        year = cols.get("year", None)
        score = None
        for k,v in cols.items():
            if "score" in k or "gpi" in k:
                score = v; break
        if year is None or score is None:
            raise ValueError("Missing Year/Score columns for GPI.")
