...
class FSIGlobalAdapter(Adapter):
    name, source = "fsi_global_avg", "Fund for Peace"
    def fetch(self) -> DataPoint:
        ...
        for k,v in cols.items():
            if "score" in k or "index" in k or "fsi" in k:
                score = v; break
        if year is None or score is None:
            raise ValueError("Missing Year/Score columns for FSI.")
